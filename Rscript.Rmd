...
# merging during.ccm with after.ccm in response. ccm
...
# merging during.ccf with after.ccf in response.ccf
...
m1.5 <- update(m1.4, .~. - scale(j.date))
summary(m1.5)


# testing the potential effect of random factors
m1.5.r1 <- update(m1.5, .~. - (1|nestid))
anova(m1.5, m1.5.r1)
m1.5.r2 <- update(m1.5, .~. - (1|observer))
anova(m1.5, m1.5.r2)
m1.5.r3 <- update(m1.5, .~. - (1|site))
anova(m1.5, m1.5.r3)
...
as.factor(df$playback.grwf)
as.factor(df$playback.grwm)
df.exp1 <- df[!is.na(df$playback.grwf),]

m3.1 <- clmm(playback.grwf ~ total.eggs
             + parasite
             + scale(mminutes)
             + scale(nestday)
             + scale(j.date)
