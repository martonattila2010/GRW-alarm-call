...
```{r m1: GRWf alarm call probability is correlated to total.eggs, parasitism and the interaction of these two}
...
m1.2 <- update(m1.1, .~. - parasite:scale(nestday))   
summary(m1.2)
...
m1.4 <- update(m1.3, .~. - scale(nestday))
summary(m1.4)
drop1(m1.4, test = "Chisq")
beep(4)

m1.5 <- update(m1.4, .~. - parasite:scale(mminutes))
summary(m1.5)
drop1(m1.5, test = "Chisq")
beep(4)

m1.6 <- update(m1.5, .~. - scale(mminutes))
summary(m1.6)
drop1(m1.6, test = "Chisq")
beep(4)

m1.7 <- update(m1.6, .~. - scale(j.date))
summary(m1.7)
...
df$boots.grwf <- as.factor(df$boots.grwf)
